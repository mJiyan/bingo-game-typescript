...
const App = () => {
  ...
  const selectSlot = (id: number): void => {
    bingoData[id] && (bingoData[id].isSelected = !bingoData[id].isSelected);
    checkBingo(id);
  ...
  const checkBingo = (index: number) => {
    ...
    for (let i = 0; i < 5; i++) {
      if (
        bingoData[xStartingIndex + i].isSelected === false &&
        xStartingIndex + i !== 12
      ) {
        isXBingo = false;
      }
      if (
        bingoData[yStartingIndex + i * 5].isSelected === false &&
        yStartingIndex + i * 5 !== 12
      ) {
        isYBingo = false;
      }
    }
  ...
  const xBingo = (index: number): boolean => {
    return xAxisBingo[Math.floor(index / 5)];
  };

  // it returns bingo for cols
  const yBingo = (index: number): boolean => {
    return yAxisBingo[index % 5];
  };
  ...
  // it returns bingo for one of the diagnol axis
  const negativeDiagnolBingo = (index: number): boolean => {
    return index % 6 === 0;
  };
  ...
  const shuffleBingoData = (array: Array<BingoType>) => {
            ...
            {bingoData &&
              bingoData.map((item, index) => {
                return (
                  (index !== 12 && (
                    <div
                      className={`border border-black p-2 sm:p-1 bg-white  `}
                    >
                      <div
                        key={index}
                        className={`flex flex-col h-32 w-32 md:h-32 md:w-32 sm:w-14 sm:h-14 ${
                          (yBingo(index) ||
                            xBingo(index) ||
                            (diagnolPositiveBingo &&
                              positiveDiagnolBingo(index)) ||
                            (diagnolNegativeBingo &&
                              negativeDiagnolBingo(index))) &&
                          "bg-mainColor"
                        } `}
                      >
                  ...
                  (index === 12 && (
                    ...
                    >
                      <div
                        key={index}
                        className="flex flex-col h-32 w-32 md:h-32 md:w-32 sm:w-14 sm:h-14 border-4 sm:border-2 border-black rounded-full"
                      >
                        <header className="flex flex-col items-end ">
